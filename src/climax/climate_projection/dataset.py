...
def load_x_y(data_path, list_simu, out_var):
    ...
    for simu in list_simu:
        ...
        if 'hist' in simu:
            ...
            # load outputs                                                             
            output_xr = xr.open_dataset(os.path.join(data_path, output_name))
            # Only apply mean if member dimension exists
            if 'member' in output_xr.dims:
                output_xr = output_xr.mean(dim='member')
            
            # Only modify pr variables if they exist
            if 'pr' in output_xr.data_vars:
                output_xr = output_xr.assign({
                    "pr": output_xr.pr * 86400,
                    "pr90": output_xr.pr90 * 86400 if 'pr90' in output_xr.data_vars else output_xr.pr * 86400
                })
            
            # Handle coordinate renaming flexibly
            rename_dict = {}
            if 'lon' in output_xr.coords:
                rename_dict['lon'] = 'longitude'
            if 'lat' in output_xr.coords:
                rename_dict['lat'] = 'latitude'
            if rename_dict:
                output_xr = output_xr.rename(rename_dict)
                
            output_xr = output_xr.transpose('time','latitude', 'longitude')
            # Only drop quantile if it exists
            if 'quantile' in output_xr.coords:
                output_xr = output_xr.drop(['quantile'])
        
        # Concatenate with historical data in the case of scenario 'ssp126', 'ssp370' and 'ssp585'
        else:
            # load inputs - check if historical file exists first
            hist_input_path = os.path.join(data_path, 'inputs_historical.nc')
            if os.path.exists(hist_input_path):
                input_xr = xr.open_mfdataset([
                    hist_input_path, 
                    os.path.join(data_path, input_name)
                ]).compute()
            else:
                # If no historical file, just use the scenario file
                input_xr = xr.open_dataset(os.path.join(data_path, input_name))
                
            # load outputs - check if historical file exists first
            hist_output_path = os.path.join(data_path, 'outputs_historical.nc')
            proj_output = xr.open_dataset(os.path.join(data_path, output_name))
            
            if os.path.exists(hist_output_path):
                hist_output = xr.open_dataset(hist_output_path)
                
                # Only apply mean if member dimension exists
                if 'member' in hist_output.dims:
                    hist_output = hist_output.mean(dim='member')
                if 'member' in proj_output.dims:
                    proj_output = proj_output.mean(dim='member')
                    
                output_xr = xr.concat([hist_output, proj_output], dim='time').compute()
            else:
                # If no historical file, just use the scenario file
                if 'member' in proj_output.dims:
                    proj_output = proj_output.mean(dim='member')
                output_xr = proj_output
